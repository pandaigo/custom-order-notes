...
// アクティブ枠から外す状態（出荷後はFreeカウント対象外）
const COMPLETED_STATUSES = new Set(['shipped', 'review']);
...
function parseOrderUrl(input) {
  ...
  // Shopify: https://admin.shopify.com/store/<store>/orders/<numeric_id>
  // 旧管理画面 https://<store>.myshopify.com/admin/orders/<id>
  const shopifyMatch =
